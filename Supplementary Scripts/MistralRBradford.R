#Import population projections for Bradford
popul.projection <- read.csv('M:\\Mistral\\PopulationProjectionBradford.csv')
...
MSOAs <- read.csv('M:\\Mistral\\LADtoMSOABradford.csv')

#Find total Bradford population for each year
iyear <- 2018
bradford.per.year <- data.frame(matrix(NA, nrow = 24, ncol = 2))
colnames(bradford.per.year) <- c('year', 'population')
for (i in 1:24){
  bradford.per.year$population[i] <- sum(popul.projection$Population[which(popul.projection$ProjectedYear == iyear)])
  bradford.per.year$year[i] <- iyear
  iyear <- iyear + 1
}

#MSOA population projections per year
LADtoMSOA <- data.frame(matrix(NA, nrow = 48, ncol = 3))
...
for (i in 1:nrow(LADtoMSOA)){
  ...
  total <- as.numeric(bradford.per.year$population[bradford.per.year$year == LADtoMSOA$year[i]])
  popul <- prop * total
  LADtoMSOA$population[i] <- as.numeric(popul)
}

# MSOAs to OAs
OAs <- read.csv('M:\\Mistral\\OAsInMSOAsBradford.csv')
...
write.csv(MSOAtoOA, file = 'M:\\Mistral\\OAprojpopulationBradford16to74.csv')
...
MSOAtoOA <- MSOAtoOA[,2:6]
write.csv(MSOAtoOA, file = 'M:\\Mistral\\OAprojpopulationBradford75plus.csv')
